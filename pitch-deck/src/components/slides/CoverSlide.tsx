...
export function CoverSlide({ index, activeIndex }: SlideComponentProps) {
  const isActive = index === activeIndex;

  return (
    <div className="relative -mx-8 -my-7 flex h-[calc(100%+3.5rem)] flex-col justify-between overflow-hidden bg-join-lime px-8 py-10 md:-mx-14 md:-my-10 md:h-[calc(100%+5rem)] md:px-14 md:py-12">
      <AnimatedText active={isActive} className="relative z-10 flex items-center justify-between">
        <JoinLogo height={20} className="opacity-90" />
        <WAFFLogo height={28} className="opacity-90" />
      </AnimatedText>
      <AnimatedText active={isActive} delay={0.15} className="relative z-10 max-w-6xl">
      ...
      <AnimatedText active={isActive} delay={0.3} className="relative z-10 flex flex-col gap-3 text-ink/70 md:flex-row md:items-end md:justify-between">
